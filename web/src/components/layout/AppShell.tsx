        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute inset-y-0 right-0 flex w-72 flex-col bg-white shadow-pop animate-slide-up">
            <div className="flex items-center justify-between border-b border-navy-100 px-4 py-4">
              <Logo />
              <button onClick={() => setDrawer(false)} className="grid h-9 w-9 place-items-center rounded-btn hover:bg-navy-50">
                <X className="h-5 w-5" />
              </button>
            </div>
...
function BottomLink({ item }: { item: NavItem }) {
  ...
  return (
    <NavLink to={item.path} className="flex flex-1 flex-col items-center justify-center gap-0.5">
      ...
      <span className={cn('text-[10px] font-semibold', active ? 'text-navy-900' : 'text-navy-400')}>{item.label}</span>
    </NavLink>
  )
}
  ...
  return (
    <div className="flex items-center gap-2 border-t border-navy-100 p-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-xs text-ink-muted">{roleLabel[user.role]}</div>
      </div>
      <button onClick={onLogout} className="grid h-10 w-10 place-items-center rounded-btn text-ink-muted hover:bg-red-50 hover:text-red-600" aria-label="خروج">
...
function CartButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className="relative grid h-11 w-11 place-items-center rounded-btn text-navy-700 hover:bg-navy-50" aria-label="عرض السعر">
